...
Connectons-nous à la page d’accueil. Nous voyons une page sous developmeent. Aucun lien ne fonctionne réellement.
...
> Bruteforce les répértoires : /api
...
La connexion à **http://hackthebox.eu:30000/api/** ne révèle rien car elle pointe vers l’index.html qui est une page vide, évidemment laissée ici pour se protéger contre la liste des répertoires.

Vérifions alors s’il y a un sous-répertoire à l’intérieur : `api`
...
Maintenant que nous en savons plus sur l’API, envoyons un paramètre. Il y a des indices évidents dans le texte de la page d’accueil sur les futures fonctionnalités attendues (connexion, réinitialisation). Testons ça:
...
> Écrivons un script python qui forcera brutalement l’ID jusqu’à ce qu’un ID valide soit trouvé:
...
while True:
    r = requests.get('http://{}:{}/api/action.php?reset={}'.format(
        host, port, id))
    if not 'Erreur: ID de compte introuvable' in r.text:
        print(r.text)
        break
    id += 1
```

Une fois le script lancé voici le résultat:
